...
def get_posts(author=None):
    ...
    if not author:
        ...
        all_posts_ids = db.session.execute(
            sql, {"author_id": author_id}).fetchall()
...
def add_post(author, title, content):
    author_id = get_id(author)
    sql = text(
        "INSERT INTO community_posts (title, content, author_id) VALUES (:title, :content, :author_id)")
    db.session.execute(
        sql, {"title": title, "content": content, "author_id": author_id})
    db.session.commit()
...
def add_comment(post_id, author, content):
    author_id = get_id(author)
    sql = text(
        "INSERT INTO post_comments (post_id, author_id, content) VALUES (:post_id, :author_id, :content)")
    db.session.execute(
        sql, {"post_id": post_id, "content": content, "author_id": author_id})
    db.session.commit()
...
def get_comment_info(comment_id):
    sql = text(
        "SELECT pc.*, u.username FROM post_comments pc JOIN users u ON pc.author_id = u.id WHERE pc.id = :id")
